...
const PlanPayment = () => {
  ...
  //api to make req to user check payment status for owner
  const checkPaymentStatus = async (planName) => {
    try {
      const { data } = await axios.put(
        `${backendUrl}/api/user/req-check`,
        {
          planName: planName,
          isRequested: true,
        },
      ...
      if (data.success) {
        toast.success("Wait for the Owner Response!");
        navigate("/");
      } else {
        toast.error("Request does not made!");
      }
    } catch (error) {
      toast.error("error to make cheeck req.!");
    }
  };
